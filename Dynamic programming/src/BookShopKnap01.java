...
public class BookShopKnap01 {//giving TLE
    ...
    public  static  int  Book(int []price,int []pages,int n,int x){
        ...
        for(int i=1;i<=n;i++)
        {
            for(int j=0;j<=x;j++)
            {
                ...
                if(j >= price[i-1])
                {
                    dp[i][j] = Math.max (dp[i][j],dp[i-1][j-price[i-1]] + pages[i-1]); //max of both the boxes included in dp[i][j].
                }
            }
        }
//        for(int i = 0; i <= n; i++)
...
>>>>>>> 1eabf1a180efd930cea20121cbbbb0e313ed35c3
